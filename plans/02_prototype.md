...
- Prepend all commands with `mise exec --` (pins typst 0.15.0, typstyle, tinymist, hk).
- Format/lint with `mise exec -- hk run fix [FILES]`; report unfixable findings from `mise exec -- hk run check [FILES]`.
...
- **Overview slides** (the common term for preview/transition/review) are first-class:
  - The presentation's 2–5 main points are declared once, centrally. A point has a **statement** (what the audience should take away) and an optional **substatement** (support for the statement).
  - `preview` and `review` slides show *all* points: statement in the accent colour, substatement below it in the foreground colour.
  - `transition` slides show the same map but mute (suppress) all points except the upcoming one, using the suppressed colour.
  - Prefer explicit declaration of points (a theme option or a dedicated function) over deriving them from headings, so statements/substatements stay decoupled from section titles. Touying's `config-store` is the right place to keep them.
...
4. **Overview slides.** Add central point declaration (statement + substatement) and implement `preview`, `transition` (with muting), and `review` on top of one shared overview layout.
...
8. **Polish & lint.** Run `mise exec -- hk run fix` on all files, then `mise exec -- hk run check` and resolve or report the remainder. Make the example a small but complete Doumont-structured deck (attention getter through close) so it doubles as documentation.

## Verification and checkpoints

The agent must not judge visual details itself. At the end of each step, compile `examples/demo.typ` to PNG/PDF (`mise exec -- typst compile`), tell the user which file/pages to look at, and ask a **yes/no question with room for explanation**, e.g. after step 2: "Open `examples/demo.pdf`, page 2 — does the content sit flush against a left margin of roughly one third of the slide width, with the message as the title? (yes/no + what looks off)". Only proceed to the next step after a yes, and fold any "no" explanation back into the current step.

Compilation success, absence of typst warnings, and clean `hk run check` output are the agent-verifiable criteria; everything visual is the human's call.
